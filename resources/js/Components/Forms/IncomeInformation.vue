<template>
    <div>
       <div>
          <h2 class="text-blue-900 dark:text-yellow-500 font-bold text-2xl mt-10 mb-3">Employer / Business  Information</h2>
          <div v-for="([key, value], index) in filteredEntries" :key="index">
             ...
             <InputText v-if="value.inputType === 'text'" class="my-3" v-model:model="incomeInformation[key]" :field="key"
                :inputType="value.inputType" @input="validator.removeError(key)" :objValidator="validator" />
             <div v-else-if="value.inputType === 'date'">
                <vue-tailwind-datepicker v-model="incomeInformation[key]" :formatter="{
             date: 'MMMM DD, YYYY',
             month: 'MMMM',
 ...
 const filteredEntries = computed(() => {
    return Object.entries(validator.value.fields).filter(
       ([key, value]) => value.type === "employer_information"
    );
 });
