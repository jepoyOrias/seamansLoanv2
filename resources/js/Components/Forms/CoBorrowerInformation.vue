<template>
    <div>
      <h2 class="text-blue-900 dark:text-yellow-500 font-bold text-2xl mt-10 mb-3">Co-Borrower Information</h2>
      <div  v-for="(coborrowerInformation, index) in coborrowersInformation" :key="index">
        ...
        <InputText v-if="value.inputType === 'text'" class="my-3" v-model:model="coborrowerInformation[key]" :field="key"
          :inputType="value.inputType" @input="validator.removeError(key)" :objValidator="validator" />
        <div v-else-if="value.inputType === 'date'">
          <vue-tailwind-datepicker  @select-month="removeError()" v-model="coborrowerInformation[key]" :formatter="{
        date: 'MMMM DD, YYYY',
        month: 'MMMM',
        ...
        'py-3 px-4 block w-full rounded-lg text-sm relative': true,
              ...
              <input v-model="selectedGender" :value="option" :type="value.inputType"
                :name="`af-account-${option}-checkbox`"
          ...
          <div class="sm:col-span-3 my-2">
            <InputText v-if="selectedGender == 'Others'" class="my-3 py-3" v-model:model="coborrowerInformation[key]"
        ...
        'py-3 px-4 block w-full rounded-lg text-sm relative': true,
              ...
              <input v-model="coborrowerInformation[key]" :value="option" :type="value.inputType"
                :name="`af-account-${option}-checkbox`"
                class="shrink-0 mt-0.5 border-gray-300 rounded-full text-blue-600 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-500 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800" />
              <span class="text-sm text-gray-500 ms-3 dark:text-gray-400">
                {{ option }}
  ...
  const coborrowersInformation = defineModel("coborrowersInformation", { default: {} });
  ...
  watchEffect(() => {
    if (selectedGender.value == "Others") {
      coborrowersInformation.value[0].gender = "";
    } else {
      coborrowersInformation.value[0].gender = selectedGender.value;
    }
  });
